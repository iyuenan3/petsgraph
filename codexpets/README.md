...
这里维护五百和飞流的公开 Codex 自定义宠物导出。每个目录都是可以安装到 Codex 的完整 v2 宠物包，包含 `pet.json` 和一张 8 列、11 行的 `spritesheet.webp`。公开包位于 `packages/public/`，私有包和制作工作区由根 Git 明确忽略。
...
完整字节数、描述文件哈希、图集尺寸和公开授权指针记录在 [`manifests/public.json`](manifests/public.json)。
...
- 更新任一文件后同步更新 `manifests/public.json` 的字节数和 SHA-256，再运行校验器。
