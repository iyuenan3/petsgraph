...
> 当前正式版本是 `1.0.0`，使用 PetPack 1.0，发布物不包含任何真实宠物素材。历史 `v0.6.0` 内嵌五百和飞流并保留旧交互，继续作为回滚事实保留。

## 开发版使用方式

1. 完整解压 `PetsGraph-v1.0.0-Windows-x64.zip`，不要只拖出 `PetsGraph.exe`。
...
- 按全部或单只卸载宠物。
- 全局 `0.5` 至 `2.0` 七档大小。
- 关于 PetsGraph，显示当前版本和 Player 与 `.petpack` 的边界。
- 退出 PetsGraph。
...
pending-import.json  仅在包激活事务未提交时存在的恢复日志
```

升级 Player 只能替换应用目录和可重建 cache，不能修改 `library/`。普通删除应用目录不会自动删除已装载宠物。导入新包或更新旧包时，Player 先写持久恢复日志，运行时窗口创建成功后才提交；验证、注册、窗口创建或进程中断失败都会恢复原版本。设置文件损坏或版本过新时采用全部隐藏的保守状态并显示警告，不会静默重置为全部可见。
...
`.NET 10` 的 45 项核心测试、WPF 零警告构建、公开合成包以及五百和飞流真实候选的原生校验已经通过。测试覆盖首装与更新回滚、进程中断恢复、事务卸载、损坏设置、长时间帧索引、真实媒体摘要和关于窗口版本格式。本轮 `v1.0.0` 没有新增真实 Windows 11 x64 GUI 复验，透明命中、DPI、拖动、托盘、多显示器、隐藏恢复、升级保留和长时间视觉表现仍需在真实 Windows 环境持续验证。
