...
| DEPLOYMENT | ✅ | macOS build 5 动作链与缓存证据、build 7 待安装候选、下一代零素材分发目标及 v0.6.0 历史基线 |
...
同步锚点指向主线程计时器优化验收 `68b9dc4`，它覆盖有界缓存 `8749ea6`、透明画布贴边 `d354f98`、素材帧率调度 `399fef5`、当前姿态贴边 `cadf095`、计时器直接 tick `cb28c6e`，并继承恢复加固 `c48cf52`、构建 4 内存修复 `9befad5`、PetPack 加固 `8b8f902`、Player 事务安全 `b417a6b`、CodexPets 原子安装 `47608a0` 和历史 Release 只读验证 `bbe8997`。远端运行 `32627927411`、`32627927365` 与 `32629083181` 成功。build 7 的安装态 CPU、四边拖动、菜单、卸载和明确视觉批准仍未完成，Windows 真实 GUI 验收暂缓，正式交付和 `1.0.0` 发布也尚未完成。
