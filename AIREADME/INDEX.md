...
| DEPLOYMENT | ✅ | macOS build 5 动作链与缓存证据、build 8 完整切换、build 9 贴边与安装态资源证据、下一代零素材分发目标及 v0.6.0 历史基线 |
...
| CHANGELOG | ✅ | 记录 2026-08-23 目录迁移、PetPack 1.0、双宠候选、双平台 Player、缓存贴边与 build 9 资源证据 |
...
同步锚点指向同包复验与 build 9 人工入口修正 `a4d6761`，它覆盖 build 9 贴边与资源验收 `974f028`、四边贴靠契约 `ded3df1`、有界缓存 `8749ea6`、素材帧率调度 `399fef5`、当前姿态贴边 `cadf095`、计时器直接 tick `cb28c6e` 和帧提交去重 `234e8fe`，并继承恢复加固 `c48cf52`、构建 4 内存修复 `9befad5`、PetPack 加固 `8b8f902`、Player 事务安全 `b417a6b`、CodexPets 原子安装 `47608a0` 和历史 Release 只读验证 `bbe8997`。远端运行 `32627927411`、`32627927365`、`32629083181`、`32630596224` 与 `32631375700` 成功。四边真实拖动、菜单、卸载和明确视觉批准仍未完成，Windows 真实 GUI 验收暂缓，正式交付和 `1.0.0` 发布也尚未完成。
