...
> last-synced: acf12b4 · 2026-08-23
> phase: macos-mechanical-acceptance-partial / windows-gui-deferred

当前公开 `v0.6.0` 仍是内嵌五百与飞流的旧架构发布版。PetPack 1.0 公开 schema、参考验证器、合成包与安全回归已经实现，五百与飞流已经形成私有机械验证候选。Apple Silicon macOS 与 Windows x64 `0.7.0-dev` 均已实现零素材 Player、持久导入激活日志、失败与重启回滚、事务卸载、保守状态恢复、独立行为时钟、固定舞台与目标菜单。Swift 25 项、MSTest 43 项和最新双平台远端运行通过，回归覆盖双宠状态、卸载中断恢复与 Windows 缓存淘汰。macOS build 5 已安装并完成双宠首轮真实自主动作链，有界缓存让双宠切换物理峰值为 126.1 MB，进入两个新稳定循环后为 46.7 MB。build 6 已实现当前姿态贴边和素材帧率调度并通过 arm64、严格签名和零素材构建，等待解锁后的安装态 CPU、拖动与视觉复验。状态栏菜单、真实拖动、卸载语义和用户正常速度视觉结论仍需人工验收，Windows 真实 GUI 验收按当前要求暂缓。后续报告仍必须显式区分 Target、机械实现、人工验收、候选包与已发布 As-built。
...
| DEPLOYMENT | ✅ | macOS build 5 动作链与缓存证据、build 6 待安装候选、下一代零素材分发目标及 v0.6.0 历史基线 |
...
| CHANGELOG | ✅ | 记录 2026-08-23 目录迁移、PetPack 1.0、双宠候选、双平台 Player、缓存贴边与帧率调度证据 |
...
同步锚点指向动作链缓存与资源优化验收 `acf12b4`，它覆盖有界缓存 `8749ea6`、透明画布贴边 `d354f98`、素材帧率调度 `399fef5`、当前姿态贴边 `cadf095`，并继承恢复加固 `c48cf52`、构建 4 内存修复 `9befad5`、PetPack 加固 `8b8f902`、Player 事务安全 `b417a6b`、CodexPets 原子安装 `47608a0` 和历史 Release 只读验证 `bbe8997`。最终代码远端运行 `32627927411` 与 `32627927365` 成功。build 6 的安装态 CPU、四边拖动、菜单、卸载和明确视觉批准仍未完成，Windows 真实 GUI 验收暂缓，正式交付和 `1.0.0` 发布也尚未完成。
