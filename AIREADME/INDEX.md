...
> last-synced: 76ad2ea · 2026-08-23
> phase: product-contract-frozen / directory-migration-in-progress

当前公开 `v0.6.0` 仍是内嵌五百与飞流的旧架构发布版。下一代零素材 Player、外部 `.petpack` 与私有 Studio 已完成产品决策但尚未实现；清理审计与 `codexpets` 目录切片已落地，其余迁移必须继续显式区分 Target 与 As-built。
...
| DIRECTORY | ✅ | 唯一项目根、公开与私有子树、已迁移 codexpets、根 Git 边界和后续迁移映射 |
| DEPLOYMENT | ✅ | 下一代零素材分发目标与 v0.6.0 双平台历史发布基线 |
| PRD | ✅ | 离世客群、定制服务、多宠独立时钟、菜单、隐藏卸载和成功指标 |
| ROADMAP | ✅ | 清理与 codexpets 已完成，继续迁移 Player、Studio、PetPack 和双平台运行时 |
...
同步锚点指向已审阅并提交的 `codexpets` 迁移切片 `76ad2ea`。AIREADME 自身随后单独提交，不把局部目录迁移伪装成新 Player 或 PetPack 已实现。
