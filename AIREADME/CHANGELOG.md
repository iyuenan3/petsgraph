...
- Changed: macOS 多宠从每个窗口一个 UI Timer 改为一个按最快活动素材帧率自适应的共享 Timer。每只宠物继续持有独立行为会话、随机状态、动作计划和切换时间，正式 24 FPS、帧序与播放倍率不变。
- Changed: 每宠各一组全局和本地鼠标监听合并为进程级各一个监听，事件仍逐次分发给全部可见宠物，拖动继续直接走 `NSView` 回调，没有鼠标节流或 Timer tolerance。
- Fixed: 宠物完成隐藏转场并进入稳定节点后清空图层、当前展示和媒体 store；全部稳定隐藏时停止共享 Timer 并移除鼠标监听。循环继续缓存全部帧，转场只保留最近一帧，避免一次性转场帧对象长期驻留。
- Added: Swift 新增共享调度最快帧率和循环与转场缓存策略回归，总数增至 32 项。两项故意破坏测试分别证明错误选择最慢帧率和转场无限缓存会被测试拦截。
- Review: build 17 为 `0.7.0-review (17)` Apple Silicon 零素材 App，主程序 1,261,168 bytes，SHA-256 为 `699baf257f1ad1d63c764f1e4ce07523b5fa411c078c1b6a6b39ffb4b1c5e813`。严格签名、合成包校验、零素材资源边界、安装升级与唯一进程启动通过，设置和双宠注册表摘要未变；build 16 已可恢复地移入系统回收站。
- Review: 五百显示、飞流隐藏时，4 个间隔 1.5 秒的画面摘要均不同。十次 CPU 采样排除首样本后平均 0.50%，上下文切换约每秒 56 次，物理占用 49.4 MB；进程只映射五百当前 236 帧循环，飞流没有 RGBA 映射或帧对象。双宠共享监听收益、全隐藏深度休眠、真实鼠标拖动与完整自主转场仍需 build 17 后续观察。
- Review: 远端 `main` 回读为 `8e6cd20`，macOS Apple Silicon 运行 `32643135161` 成功，完整执行 33 项 PetPack 回归、32 项 Swift 测试、零素材 arm64 App 构建、严格签名和代码产物上传。
...
- Added: `feaf6cd` 为 macOS 导出 `com.maxwell.petsgraph.petpack` 文件类型，访达双击与状态栏文件选择共用同一套验证、原子装载、激活和失败回滚逻辑；CI 同时检查 App 的文件类型声明。
- Review: build 11 为 `arm64` 零素材 App，主程序 1,216,176 bytes，SHA-256 为 `a934b9c463ac2ea693549e3fc77a7d33656041ef234e0ae723036480b4dab31f`；构建后与安装后严格签名通过。原 build 10 已可恢复地保存到系统回收站。
- Review: build 11 直接打开五百与飞流两个原始 `.petpack` 后，内部注册表正好包含 2 个包，canonical copy 的 SHA-256 分别为 `14f719b...` 与 `f0308cd...`，逐字节匹配原件；最近装载目录同步记为飞流候选目录。用户明确确认两只宠物已经装载。
- Review: 用户继续确认双宠显示和隐藏已经通过。当前设置回读两只宠物均为显示状态，历史重启证据继续覆盖单只隐藏与双宠显示恢复。
- Fixed: `5e7f516` 关闭主菜单及各子菜单的 AppKit 自动启用，避免系统重新校验覆盖运行态 `isEnabled`，使已经显示的宠物在“显示宠物”中禁用、已经隐藏的宠物在“隐藏宠物”中禁用，对应“全部”遵循同一规则。
- Review: build 12 为 `arm64` 零素材 App，主程序 1,216,224 bytes，SHA-256 为 `a97fe91d46b2b6f8c049359021c7c2506be980e1516a37e3e41ea3b995e3f361`；26 项 Swift 测试、严格格式、构建后与安装后签名均通过，升级后内部资料库仍为五百与飞流两个包。灰显效果仍需用户在真实状态栏菜单确认。
- Review: `feaf6cd` 的 macOS 运行 `32632984632` 与 `5e7f516` 的运行 `32633124523` 均成功，远端分别验证文件关联声明、菜单灰显修复、26 项 Swift 测试、arm64 签名和零素材 App 构建。
- Review: build 12 双宠无装载弹窗稳定态中，`top` 首个初始化样本为 0.0%，后 9 次 CPU 平均为 0.99%；物理占用为 53.6 MB、峰值 54.0 MB，只映射飞流与五百各自当前循环，启动后没有新的崩溃报告。该结果取代 build 11 装载结果窗口期间的 65.9 MB 作为当前日常稳定态参考。
...
- Review: `901b364` 的远端 macOS Apple Silicon 运行 `32636187517` 成功，复验 33 项 PetPack 回归、27 项 Swift 测试、arm64 零素材 App 和严格签名。
- Review: build 14 双宠稳定运行时，十次 CPU 取样为 `0.0, 0.5, 0.9, 0.8, 0.8, 0.8, 0.8, 0.7, 0.7, 0.7`，排除首个样本后平均 0.74%；物理占用为 53.3 MB、峰值 56.0 MB，只映射飞流和五百各自当前循环。位置持久化修复没有造成可见资源回退。
- Review: 用户在 build 14 上确认上下左右四边贴靠、`0.5` 至 `2.0` 七档全局大小和重启保持全部通过。四边限位、统一倍率与实际锚点持久化不再是当前人工门禁。
- Fixed: `d0cc03e` 将 macOS 透明像素命中判断提取到核心模块，固定屏幕坐标到顶部原点画布坐标的映射、alpha 阈值和异常数值保守处理。新增两项回归后 Swift 测试增至 29 项，远端 macOS 运行 `32637170614` 成功。
- Fixed: `5fdc03f` 删除大小菜单的 `%.2g` 有效数字格式化，七个离散倍率改为显式稳定标签，确保 `1.25×` 与 `1.75×` 不再显示为 `1.2×` 与 `1.8×`。新增标签回归后 Swift 测试增至 30 项，远端 macOS 运行 `32638115738` 成功。
- Review: build 16 为 `arm64` 零素材 App，版本 `0.7.0-review`、构建号 16，主程序为 1,222,864 bytes，SHA-256 为 `45a1c4f0776c71b63d956ce2bda8c786d23f58cb308fb315cd501453524cdb4d`；构建物与安装物摘要一致且严格签名通过。build 15 已可恢复地移入 `~/.Trash/PetsGraph-build15-20260823-200200.app`。build 16 首次 GUI 启动、菜单标签和透明命中仍需实际确认。
- Fixed: `0d06e42` 把 Windows 大小菜单从区域格式化改为七个显式稳定标签，确保双平台都固定显示 `0.5×`、`0.75×`、`1.0×`、`1.25×`、`1.5×`、`1.75×`、`2.0×`。新增回归后 MSTest 增至 44 项，远端 Windows 运行 `32638713180` 成功。
- Review: 完整重构审计再次通过 PetPack 33 项、Codex 宠物 3 项、Swift 30 项、MSTest 44 项和双平台严格格式检查；Windows 全解决方案 Release 构建为 0 警告、0 错误。Python、Swift build 16 与 C# 对五百、飞流同一候选包给出一致的 SHA-256、clip、node 与 edge 数。同包机械一致性完成，真实桌面视觉一致性仍保留人工门禁。
- Review: 当前 Windows x64 零素材审计 ZIP 为 75,278,960 bytes，SHA-256 为 `b5981c7c61186aa18481ebb0b30252143cd923fc197759eb27c36be77c139b84`，主程序为 AMD64 PE，远端 Windows 运行 `32638713180` 成功。较早清理记录中的两个原始回收站路径目前无法解析，因此不再承诺按原路径恢复；build 15 的精确回收站路径仍可解析。
- Review: 用户明确允许后，build 16 完成首次真实启动。唯一 PID 97269 的运行二进制摘要与安装记录一致且严格签名通过，进程恢复五百与飞流两个 canonical 包。五百 4 个正常速度画面摘要持续变化；十次 CPU 取样排除首样本后平均约为 0.79%，内存列稳定为 52 MB，探测前后设置文件摘要完全一致。
