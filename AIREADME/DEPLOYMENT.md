...
- 用户在 build 11 上确认显示和隐藏通过。`5e7f516` 与 build 12 关闭菜单自动启用，确保手工计算的当前状态不会被 AppKit 覆盖；升级后回读内部资料库仍为五百和飞流两个包，当前两只均显示。灰显结果仍需用户打开真实状态栏菜单确认。`5e7f516` 的 macOS 运行 `32633124523` 成功。
