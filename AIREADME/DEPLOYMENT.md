...
- build 10 在系统装载窗口显示文件夹双击和 `⌘⇧G` 提示，首次使用从下载目录开始，成功选择后记忆上次目录。用户已确认卸载成功，安装 build 10 后回读 `registry.json` 为 0 个宠物，`Library`、`Cache` 与 `Staging` 均为 0 个文件；五百和飞流原始候选仍在用户保存位置。卸载后重新装载与窗口提示仍需用户目视确认。
