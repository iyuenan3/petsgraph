# CORE：petsgraph

## 身份

PetsGraph 是为 Maxwell 自己和朋友们熟悉的真实猫狗制作的桌面陪伴项目。App、宠物身份和行为配置相互分离，一个 App 进程可以同时装载多只独立宠物。当前 macOS Apple 芯片版和 Windows 11 x64 便携版均为 `v0.6.0`。两个平台均使用五百和飞流的同一份 schema `0.4.0`、内容版本 `0.5.10` 宠物包，用户界面只显示不带姓氏的熟悉短名。第一款体验仍是安静睡眠陪伴，核心不是功能数量，而是熟悉的宠物长时间自然待在桌面上。

## 使命

让 Maxwell 和朋友们在工作时获得熟悉宠物的安静陪伴。每只宠物都必须保持可辨认的身份、自然的承重和连续的动作关系，而不是被套进同一组通用姿势的视频贴纸。

生产工具会继续复用照片管理、provider 调用、抠图、验收、履历和打包能力，但不再承诺把它们做成面向所有用户的一键通用素材生成 Skill。新增宠物采用人工定制：先研究该宠物真实习惯，再为它单独设计姿态、道具、动作图和行为参数。

## 当前交付边界

- macOS 公开版与 Windows 11 x64 便携版均支持本地多宠、每包一只宠物、一个 App 进程装载零只、一只或多只宠物，运行时完全离线。
- 每只已装载宠物默认睡眠，并在自己的合法动作图和独立随机时钟上按随机长尾时间换姿，不做固定串行轮播、跨宠同步或随机硬切。
- 点击宠物在当前视觉场景的睡眠与正面坐姿之间切换。带道具场景必须有保留同一道具的专用坐姿。
- 菜单栏可以独立装载或隐藏每只宠物，并用中文名称指定该宠物包声明的睡姿。未装载宠物的姿态菜单禁用，重新装载从默认睡姿开始。当前不可中断动作播放完成后再执行最后一次选择。
- 用户可以在整个桌面范围拖动宠物，宠物始终位于普通窗口和 Dock 前方。
- 首次安装默认装载全部内嵌宠物。默认在物理屏幕左下角横排，五百左一、飞流左二，窗口不重叠。之后分别持久化每只宠物的装载状态和位置。
- 全局宠物大小提供 `0.5×`、`0.75×`、`1.0×`、`1.25×`、`1.5×`、`1.75×`、`2.0×` 七档。一次调整作用于全部已装载宠物并持久化，缩放保持当前动作、当前地面锚点和宠物之间的既定相对体型。
- 睡眠内部 root motion 严格为零。只有进入或离开枕头、猫窝等道具场景的已批准短步动作可以产生水平 root motion。
- App 名称固定为 `PetsGraph`，菜单从每个宠物包读取宠物名称、场景和中文姿态。v0.6.0 两个平台同时内嵌五百和飞流 `0.5.10` 宠物包，两只宠物均已通过真实桌面验收。
- 仓库另行维护五百 `wubai-v0` 与飞流 `feiliu-hatch-native-v1` 两套 Codex v2 自定义宠物导出。它们安装到 Codex 自己的 Pets 系统，不由 PetsGraph App 加载，也不继承 `.petsgraph-pet` 的动作图、连续视频或运行时批准状态。
- 飞流采用无道具与猫窝两个场景，当前动作图为 5 个无道具睡姿、4 个猫窝睡姿和 2 个点击坐姿。`rest.floor.prone.right` 是地面枢纽，连接地面正面坐姿、侧身伸展睡、紧蜷睡、半仰睡和猫窝蜷睡；仰躺睡只经半仰睡往返。`rest.cat-bed.curled` 是猫窝枢纽，连接猫窝正面坐姿、猫窝自然趴睡、猫窝侧伸睡和猫窝舒展露腹睡。两个坐姿不进入自主睡姿随机池，收爪趴睡与睡眠香箱不进入当前图。
- 新宠物只按 Maxwell 自有宠物或朋友明确委托的定制项目推进，不以姿势数量、动作图或道具的跨宠物一致为目标。

## Non-Goals（明确不做）

- 首个 MVP 不开放点击桌面目的地移动，不自主巡游，不把走路或跑步作为默认行为。
- 首个 MVP 不做玩耍、进食、舔毛、养成数值、任务提醒或 Agent 能力。
- 第一阶段不做宠物互动、宠物关系、自动避让、碰撞、Intel Mac、Windows on Arm、Windows 10、Linux 或跨显示器自主移动。用户手动拖动后允许宠物窗口重叠。
- 不做云账号、素材市场、付费代生成、托管额度或运行时联网生成。
- 不做任意用户上传照片后自动产出完整宠物包的通用自助产品，也不以一套固定十姿势模板约束所有宠物。
- 不监听宠物之外的全桌面鼠标点击，不以辅助功能权限作为 MVP 前提。
- 不把骨骼建模、Hatch Pet、肢体拼接、RIFE、光流、交叉淡化或自动补帧作为正式素材路线。
- 不以动态 WebP 作为未经验证的唯一运行时格式。

已完成的走路、跑步和 root motion 素材不删除、不降级，也不混入首个 MVP 的默认行为。它们作为后续可选活动能力继续保留。

## 绝不 / Hard Constraints

- 真实宠物身份、动作质量和陪伴感优先于动作数量、互动数量与生成次数。
- 正式动作来自直接生成视频的连续原始帧。确定性后处理可以抠图、统一画布、固定重定位和编译，但不得伪造承重或补造动作。
- 生成可以在用户明确授权下进行多次受控尝试。每次都必须有新的失败假设或约束变化，完整保留任务、输入、提示词、seed、结果与拒绝原因；禁止无差别批量抽卡。
- 素材身份、动作自然度、接触关系、透明边缘和切换质量最终由人工视觉验收。机械检查只能预筛。
- 运行时只能沿动作图切换。普通行为等待安全退出，预加载下一条边和目标循环，有向过渡按原帧顺序完整播放。
- 睡姿随机调度只能选择允许长期停留的节点，并受场景粘性、近期去重、节点权重和活动冷却约束。网关、交互节点和过渡边不能被当作普通睡姿随机抽取。
- 枕头是核心场景的一部分。进入枕头场景后，枕头的位置、尺度、方向和接触关系必须连续；点击、坐起和拖动不能让枕头突然消失。
- 猫窝是飞流核心场景的一部分。猫和猫窝作为同一组已批准画面播放；同一条过渡路径只能使用一个固定空间变换，不能逐帧重新定位或在运行时临时拼接猫窝。
- 点击只作用于宠物本体。枕头和透明画布不应扩大点击命中区域。
- 用户拖动属于强操控，可以在全桌面改变 x 和 y。自主动画和场景进出最多只改变 x，不能偷偷改写用户放置的 y。
- 独立动作通过不等于可安装。动作、图边、完整场景链和真实桌面长时间运行必须分别验收。
- 宠物包只包含数据与媒体，不执行包内脚本，不携带生成凭据、原始私密照片、签名 URL 或 provider 配置。
- Codex 图集的 8×11 结构、透明通道和哈希通过只表示机械契约成立。其动作连贯性、身份和真实窗口观感必须单独人工观看，不能写成 PetsGraph 正式素材通过。
- provider 只属于私有制作侧。Seedance 负责连续动作视频，静态身份图、姿态锚点和安全画布参考可使用 Seedream 或 `gpt-image-2`；任何 provider 都不能直接绕过人工验收进入正式宠物包。
- PNG/RGBA 序列是制作事实源；图集是可重建运行时产物；WebP、GIF 和 MP4 只用于预览或导出。
- 低功耗运行时副本只能从已批准 PNG 包确定性编译。每条 clip 只允许一个固定裁剪矩形，不能逐帧重新定位、缩放或修改批准源帧。
- 粗抠阶段一旦完成人工通过的整段缩放与平移，精抠只能改变透明度和边缘颜色，不能重新拟合位置或尺度。已经干净的素材保持逐字节不变，只选择性重做确有毛边或主体保护问题的片段。

## 生命周期

`public-dual-platform / petsgraph-0.6.0` 是当前发布阶段。macOS 最新产物是 v0.6.0 Apple 芯片 DMG，Windows 最新产物是 v0.6.0 Windows 11 x64 ZIP。通用 App 统一使用 `PetsGraph` 品牌和双猫相伴 Logo，macOS Bundle ID 为 `com.maxwell.petsgraph`，两个平台都内嵌 `wubai-quiet-companion-0.5.10` 与 `feiliu-quiet-companion-0.5.10`。两个包均为 schema `0.4.0`、`cropped-rgba-clips`、`runtime-chain-approved` 和 `installable=true`。

五百包包含 53 个 clip、14 个节点、39 条边与 6,866 帧。飞流包包含 31 个 clip、11 个节点、20 条边与 5,147 帧。合计 84 个 clip 与 12,013 帧。67 项 XCTest、完整包和媒体校验、arm64 架构、ad-hoc 签名、双宠首次布局、独立随机时钟、菜单、拖动、七档缩放、猫窝路径、透明边缘和相对体型已通过。飞流 `baseHeightPt=181.125`，五百 `baseHeightPt=172.5`，即同倍率下飞流比五百大 5%。

macOS 版只构建 Apple 芯片 `arm64`，最低 macOS 14。App 使用 ad-hoc 签名，没有 Developer ID 签名和 Apple 公证，朋友首次打开时需要通过 Finder 右键「打开」或「隐私与安全性」确认。正式媒体内嵌在 App 中，v0.6.0 Release 提供一个 macOS DMG 和一个 Windows ZIP，不提供 App ZIP、独立宠物包、预览图或校验和附件。仓库双平台清单固定记录两个附件的字节数和 SHA-256。

v0.6.0 macOS App 解压后约 2.4 GiB，DMG 为 `857640594` 字节，SHA-256 为 `04c6f30e35d7dd8b5b096d0051aad628987d59b8245d84060cf704f02709b159`。运行时继续以存储换取较低 CPU 与内存开销，并使用每用户单实例锁。v0.4.0 的五百实测资源结果仍是参考基线，双宠整日 CPU 与内存属于发布后的持续观察项，不是本次已通过视觉与功能链的替代结论。

Windows `v0.6.0` 只面向 Windows 11 x64，使用 .NET 10 WPF 和 self-contained 便携 ZIP，不使用 MSIX、代码签名或安装器。它复用同一份五百与飞流 `cropped-rgba-clips` 包，支持透明置顶窗口、宠物本体点击、拖动、托盘菜单、七档全局缩放、位置持久化和每用户单实例。冻结 ZIP 为 `913953281` 字节，SHA-256 为 `90578d6620ef9c221c173b173c24631d6e756b372b532030f8669994d22b0015`。本机交叉编译、双宠完整性、GitHub Windows Runner 与真实 Windows 11 x64 朋友使用验收均已完成，朋友最终反馈为没有问题，发布所有者明确授权发布 v0.6.0。该真机反馈没有附带分项时长或 DPI 测试日志，因此只记录最终人工结论，不补写未留证的量化结果。

2026-08-19 起，Git 历史在 `codex-pets/` 维护合计约 4.4 MB 的两套 Codex 导出、schema 1 哈希清单和安全安装工具。五百图集 SHA-256 为 `260a092e7bef78ad2005c766ed5c9c0e4cf48195ce4d99c86b5f2839cfbf5624`，飞流图集 SHA-256 为 `20fc7e6a065bffba85e3d592b3f027fe6e668756790a57b1a61fa3ca7f52177a`。这项仓库分发不改变 v0.6.0 Release 的双附件集合。
