...
13. 已用公开 store、前向兼容与 deflate 测试包、五百和飞流真实候选验证 Swift 原生加载器，并完成严格编译、11 项测试、arm64 App、ad-hoc 签名和零素材检查。真实桌面观看尚未通过，不能提升为发布结论。
...
17. 远端 push 与修复复跑已经完成。PetPack 与 Windows 工作流分别发现 Python ZIP 路径规范化和 .NET 测试包创建主机差异，已经用 `orig_filename` 与确定性中央目录元数据修复；远端 `main` 回读为 `97485a0`，PetPack、macOS、Windows 三条工作流及两个代码开发产物上传均通过。
...
3. 补齐更新失败回滚、应用升级、多宠 2、4、8 只并发和同时过渡的机械与人工回归，记录两个平台的 CPU、内存、卡帧、闪动和连接观察，不能用单元测试替代视觉结论。
